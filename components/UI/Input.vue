<template>
    <div>
        <label :for="props.label" v-if="props.label" class="block pl-3 ml-px text-sm font-medium text-gray-700">{{ props.label
        }}</label>
        <div class="mt-1">
            <input :value="props.modelValue" @input="event => emits('update:modelValue', event.target.value)"
                ...
                :type="props.type" :placeholder="props.placeholder" :name="props.label">
        </div>
    </div>
</template>
<script setup>
